<?php
    ...
    if(isset($_GET['dlt'])){
        $c_id = $_GET['dlt'];
        $dlt = mysqli_query($conn, "DELETE FROM customer_info WHERE customer_id = '$c_id'");
    }
...
<body class="m-0 p-0 font-pop">  
    ...
    <div class="w-4/5">
        ...
        <div class=" mt-5 text text-center"> 
            ...
            <table class=" mt-4 table-auto text-center mx-auto bg-navy text-white border  border-white border-collapse">
                <thead>
                    <tr class="">
                        <th class="p-4 border  border-white">Customer ID</th>
                        <th class="p-4 border  border-white">Customer Name</th>
                ...
                <tbody class="">
                    <tr class="">
                        <td class="p-4 border  border-white"><?php echo $row['customer_id']?></td>
                    <td class="p-4 border  border-white"><?php echo $row['c_name']?></td>
                    <td class="p-4 border  border-white"><?php echo $row['c_phone']?></td>
 ...
 <!--                       <a class="inline-block rounded-lg py-1 px-2 font-semibold hover:outline hover:ouline-navy hover:text-white   hover:bg-navy text-navy bg-white duration-700 ease-in-out" href="#">Edit</a>-->
                        <a class="inline-block rounded-lg py-1 px-2 font-semibold hover:outline hover:ouline-navy hover:text-white   hover:bg-navy text-navy bg-white duration-700 ease-in-out" href="customerdetails.php?dlt=<?php echo $row['customer_id']?>">Delete</a>
                    </td>
                    </tr>
                </tbody>
                <?php
